...
public class MatrixDeterminantSupplier extends SettingsProblemSupplier {
	...
	public Problem get() {
		...
		if(Math.random() <= 0.5) { //ask them to find the determinant
			return MultiValued.of(ensureMath(det(m) + op('='))).setLines(2).addResult(det);
		}
			...
			return MultiValued.of(ensureMath(det(strs) + op('=') + frac(det))).setLines(2).addResult(missingValue);
		}
	}
